...
import com.app.cms.manager.assist.CmsKeywordMng;
...
import com.app.common.page.Paginable;
import com.app.common.page.SimplePage;
...
public class DynamicPageAct {
	...
	/**
	 * 后加：匹配子文根访问网站，支持易哥域名访问多个网站 
	 * eg:http://ip:port/contextPath/sitePath/index.*
	 *    http://ip:port/contextPath/sitePath2/index.*
	 * @param request
	 * @param model
	 * @return
	 */
	@RequestMapping("/*/index.*")
	public String indexSon(HttpServletRequest request, ModelMap model) {
		CmsSite site = CmsUtils.getSite(request);
		FrontUtils.frontData(request, model, site);
		if(site.getStaticIndex()){
			if(site.getIndexToRoot()){
				return "index" + site.getStaticSuffix();
			}
			return site.getStaticDir() + "/index" + site.getStaticSuffix();
		}
		...
		if (len == 1) {
			...
			if (paths[2].equals(INDEX)) {
				...
				try {
					...
					log.debug("Content id must String: {}", paths[2]);
					return FrontUtils.pageNotFound(request, response, model);
				}
			}
		}else {
